    ...
    where TUpdateDto : class
{
    ...
    public virtual async Task<Result<TDto>> UpdateAsync(TKey id, TUpdateDto updateDto, CancellationToken ct = default)
    {
        ...
        if (entity == null)
        {
            ...
            return Result<TDto>.Failure("NotFound", $"Registro não encontrado: {id}");
        }
    ...
    public virtual async Task<Result> DeleteAsync(TKey id, CancellationToken ct = default)
    {
        ...
        if (entity == null)
        {
            Logger.LogWarning("{Entity} não encontrado para exclusão: {Id}",
                typeof(TEntity).Name, id);
            return Result.Failure(new Error("NotFound", $"Registro não encontrado: {id}"));
        }
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure(new Error("General.Error", $"Erro ao excluir registro: {ex.Message}"));
        }
    }
