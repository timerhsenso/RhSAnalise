...
/// <summary>
/// Implementação do serviço de API para Tipo Fornecedor.
...
/// </remarks>
public class SGC_TipoFornecedorApiService : ISGC_TipoFornecedorApiService
{
    private readonly HttpClient _httpClient;
    private readonly IHttpContextAccessor _httpContextAccessor;
    private readonly ILogger<SGC_TipoFornecedorApiService> _logger;
    private readonly JsonSerializerOptions _jsonOptions;

    private const string ApiRoute = "api/gestaodepessoas/sgc_tipofornecedores";

    public SGC_TipoFornecedorApiService(
        HttpClient httpClient,
        IHttpContextAccessor httpContextAccessor,
        ILogger<SGC_TipoFornecedorApiService> logger)
    {
    ...
    private async Task SetAuthHeaderAsync()
    {
        ...
        if (context?.User?.Identity?.IsAuthenticated == true)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                _logger.LogDebug("[SGC_TIPOFORNECEDOR] Token JWT configurado");
            }
            else
            {
                _logger.LogWarning("[SGC_TIPOFORNECEDOR] Token JWT não encontrado");
            }
        }
        else
        {
            _logger.LogWarning("[SGC_TIPOFORNECEDOR] Usuário não autenticado");
        }
    }
        ...
        string operation)
    {
        ...
        _logger.LogDebug("[SGC_TIPOFORNECEDOR] {Op} - Status: {Status}, Content: {Content}", 
        ...
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro JSON em {Op}", operation);
    ...
    public async Task<ApiResponse<PagedResult<SGC_TipoFornecedorDto>>> GetPagedAsync(
        int page, 
        int pageSize, 
        string? search = null)
    {
        try
        {
            ...
            _logger.LogDebug("[SGC_TIPOFORNECEDOR] GET {Route}{Query}", ApiRoute, query);

            var response = await _httpClient.GetAsync($"{ApiRoute}{query}");
            return await ProcessResponseAsync<PagedResult<SGC_TipoFornecedorDto>>(response, "GetPaged");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em GetPagedAsync");
            return Fail<PagedResult<SGC_TipoFornecedorDto>>("Erro de conexão com o servidor");
        }
        catch (TaskCanceledException)
        {
            return Fail<PagedResult<SGC_TipoFornecedorDto>>("Tempo limite excedido");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em GetPagedAsync");
            return Fail<PagedResult<SGC_TipoFornecedorDto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<IEnumerable<SGC_TipoFornecedorDto>>> GetAllAsync()
    {
        try
        {
            await SetAuthHeaderAsync();
            var response = await _httpClient.GetAsync($"{ApiRoute}?page=1&pageSize=10000");
            var result = await ProcessResponseAsync<PagedResult<SGC_TipoFornecedorDto>>(response, "GetAll");
            
            if (result.Success && result.Data != null)
                return Success<IEnumerable<SGC_TipoFornecedorDto>>(result.Data.Items);
            
            return Fail<IEnumerable<SGC_TipoFornecedorDto>>(result.Error?.Message ?? "Erro ao buscar dados");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em GetAllAsync");
            return Fail<IEnumerable<SGC_TipoFornecedorDto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<SGC_TipoFornecedorDto>> GetByIdAsync(int id)
    {
        try
        {
            await SetAuthHeaderAsync();
            var response = await _httpClient.GetAsync($"{ApiRoute}/{id}");
            return await ProcessResponseAsync<SGC_TipoFornecedorDto>(response, "GetById");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em GetByIdAsync");
            return Fail<SGC_TipoFornecedorDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em GetByIdAsync");
            return Fail<SGC_TipoFornecedorDto>(ex.Message);
        }
    }

    public async Task<ApiResponse<SGC_TipoFornecedorDto>> CreateAsync(CreateSGC_TipoFornecedorRequest request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[SGC_TIPOFORNECEDOR] POST {Route} - Body: {Body}", ApiRoute, json);
            
            var response = await _httpClient.PostAsync(ApiRoute, content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<SGC_TipoFornecedorDto>(response, "Create");
            ...
            return Fail<SGC_TipoFornecedorDto>(createResult?.Error?.Message ?? "Erro ao criar registro");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em CreateAsync");
            return Fail<SGC_TipoFornecedorDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em CreateAsync");
            return Fail<SGC_TipoFornecedorDto>(ex.Message);
        }
    }

    public async Task<ApiResponse<SGC_TipoFornecedorDto>> UpdateAsync(int id, UpdateSGC_TipoFornecedorRequest request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[SGC_TIPOFORNECEDOR] PUT {Route}/{Id} - Body: {Body}", ApiRoute, id, json);
            
            var response = await _httpClient.PutAsync($"{ApiRoute}/{id}", content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<SGC_TipoFornecedorDto>(response, "Update");

            return await GetByIdAsync(id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em UpdateAsync");
            return Fail<SGC_TipoFornecedorDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em UpdateAsync");
            return Fail<SGC_TipoFornecedorDto>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(int id)
    {
        ...
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em DeleteAsync");
            return Fail<bool>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em DeleteAsync");
    ...
    public async Task<ApiResponse<bool>> DeleteMultipleAsync(IEnumerable<int> ids)
    {
        ...
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em DeleteMultipleAsync");
            return Fail<bool>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[SGC_TIPOFORNECEDOR] DELETE {Route}/batch - Body: {Body}", ApiRoute, json);
        ...
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Erro de conexão em DeleteBatchAsync");
            return Fail<BatchDeleteResultDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SGC_TIPOFORNECEDOR] Exceção em DeleteBatchAsync");
