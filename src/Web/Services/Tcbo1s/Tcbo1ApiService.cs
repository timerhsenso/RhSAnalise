...
/// <summary>
/// Implementação do serviço de API para Tabela de Ocupação.
...
/// </remarks>
public class Tcbo1ApiService : ITcbo1ApiService
{
    private readonly HttpClient _httpClient;
    private readonly IHttpContextAccessor _httpContextAccessor;
    private readonly ILogger<Tcbo1ApiService> _logger;
    private readonly JsonSerializerOptions _jsonOptions;

    private const string ApiRoute = "api/gestaodepessoas/tcbo1s";

    public Tcbo1ApiService(
        HttpClient httpClient,
        IHttpContextAccessor httpContextAccessor,
        ILogger<Tcbo1ApiService> logger)
    {
    ...
    private async Task SetAuthHeaderAsync()
    {
        ...
        if (context?.User?.Identity?.IsAuthenticated == true)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                _logger.LogDebug("[TCBO1] Token JWT configurado");
            }
            else
            {
                _logger.LogWarning("[TCBO1] Token JWT não encontrado");
            }
        }
        else
        {
            _logger.LogWarning("[TCBO1] Usuário não autenticado");
        }
    }
        ...
        string operation)
    {
        ...
        _logger.LogDebug("[TCBO1] {Op} - Status: {Status}, Content: {Content}", 
        ...
        {
            _logger.LogError(ex, "[TCBO1] Erro JSON em {Op}", operation);
    ...
    public async Task<ApiResponse<PagedResult<Tcbo1Dto>>> GetPagedAsync(
        int page, 
        int pageSize, 
        string? search = null)
    {
        try
        {
            ...
            _logger.LogDebug("[TCBO1] GET {Route}{Query}", ApiRoute, query);

            var response = await _httpClient.GetAsync($"{ApiRoute}{query}");
            return await ProcessResponseAsync<PagedResult<Tcbo1Dto>>(response, "GetPaged");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em GetPagedAsync");
            return Fail<PagedResult<Tcbo1Dto>>("Erro de conexão com o servidor");
        }
        catch (TaskCanceledException)
        {
            return Fail<PagedResult<Tcbo1Dto>>("Tempo limite excedido");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em GetPagedAsync");
            return Fail<PagedResult<Tcbo1Dto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<IEnumerable<Tcbo1Dto>>> GetAllAsync()
    {
        try
        {
            await SetAuthHeaderAsync();
            var response = await _httpClient.GetAsync($"{ApiRoute}?page=1&pageSize=10000");
            var result = await ProcessResponseAsync<PagedResult<Tcbo1Dto>>(response, "GetAll");
            
            if (result.Success && result.Data != null)
                return Success<IEnumerable<Tcbo1Dto>>(result.Data.Items);
            
            return Fail<IEnumerable<Tcbo1Dto>>(result.Error?.Message ?? "Erro ao buscar dados");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em GetAllAsync");
            return Fail<IEnumerable<Tcbo1Dto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tcbo1Dto>> GetByIdAsync(string id)
    {
        try
        {
            await SetAuthHeaderAsync();
            var response = await _httpClient.GetAsync($"{ApiRoute}/{id}");
            return await ProcessResponseAsync<Tcbo1Dto>(response, "GetById");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em GetByIdAsync");
            return Fail<Tcbo1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em GetByIdAsync");
            return Fail<Tcbo1Dto>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tcbo1Dto>> CreateAsync(CreateTcbo1Request request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[TCBO1] POST {Route} - Body: {Body}", ApiRoute, json);
            
            var response = await _httpClient.PostAsync(ApiRoute, content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<Tcbo1Dto>(response, "Create");
            ...
            return Fail<Tcbo1Dto>(createResult?.Error?.Message ?? "Erro ao criar registro");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em CreateAsync");
            return Fail<Tcbo1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em CreateAsync");
            return Fail<Tcbo1Dto>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tcbo1Dto>> UpdateAsync(string id, UpdateTcbo1Request request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[TCBO1] PUT {Route}/{Id} - Body: {Body}", ApiRoute, id, json);
            
            var response = await _httpClient.PutAsync($"{ApiRoute}/{id}", content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<Tcbo1Dto>(response, "Update");

            return await GetByIdAsync(id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em UpdateAsync");
            return Fail<Tcbo1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em UpdateAsync");
            return Fail<Tcbo1Dto>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(string id)
    {
        ...
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em DeleteAsync");
            return Fail<bool>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em DeleteAsync");
    ...
    public async Task<ApiResponse<bool>> DeleteMultipleAsync(IEnumerable<string> ids)
    {
        ...
        {
            _logger.LogError(ex, "[TCBO1] Exceção em DeleteMultipleAsync");
            return Fail<bool>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<string> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[TCBO1] DELETE {Route}/batch - Body: {Body}", ApiRoute, json);
        ...
        {
            _logger.LogError(ex, "[TCBO1] Erro de conexão em DeleteBatchAsync");
            return Fail<BatchDeleteResultDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TCBO1] Exceção em DeleteBatchAsync");
