/**
 * ============================================================================
 * TIPO FORNECEDOR - JavaScript com Controle de Permissões
 * ============================================================================
 * Arquivo: wwwroot/js/sgc_tipofornecedores/sgc_tipofornecedor.js
 * Versão: 3.1 (Suporte a PKs de texto)
 * Gerado por: GeradorFullStack v3.1
 * Data: 2025-12-02 15:55:54
 * 
 * Implementação específica do CRUD de Tipo Fornecedor.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class SGC_TipoFornecedorCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [SGC_TipoFornecedor] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [SGC_TipoFornecedor] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        ...
        console.log('📤 [SGC_TipoFornecedor] Dados a enviar:', formData);
        return formData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [SGC_TipoFornecedor] Registro salvo:', data);
    }
     ...
     */
    getRowId(row) {
        const id = row[this.config.idField] || row.idTipoFornecedor || row.IdTipoFornecedor || row.id || row.Id || '';
...
$(document).ready(function () {
    ...
    console.log('🔐 [SGC_TipoFornecedor] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        let id = row[fieldName] || row[fieldName.toLowerCase()] || row[fieldName.toUpperCase()] || 
                 row['idTipoFornecedor'] || row['IdTipoFornecedor'] || row['id'] || row['Id'] || '';
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [SGC_TipoFornecedor] ID vazio para row:', row);
        }
    ...
    const columns = [
        // Coluna de seleção (checkbox)
        {
            ...
            render: function (data, type, row) {
                ...
                if (window.crudPermissions.canDelete) {
                    const id = getCleanId(row, 'idTipoFornecedor');
        ...
        // Coluna de ações
        {
            ...
            render: function (data, type, row) {
                ...
                console.log('🔧 [SGC_TipoFornecedor] Renderizando ações | ID:', id, '| Row:', row);
    ...
    window.sgc_tipofornecedorCrud = new SGC_TipoFornecedorCrud({
        controllerName: 'SGC_TipoFornecedores',
        entityName: 'Tipo Fornecedor',
        idField: 'idTipoFornecedor',
        columns: columns,
    ...
    if (!window.crudPermissions.canCreate) {
        ...
        console.log('🔒 [SGC_TipoFornecedor] Botão "Novo" desabilitado (sem permissão de criação)');
    }
    ...
    if (!window.crudPermissions.canDelete) {
        ...
        console.log('🔒 [SGC_TipoFornecedor] Botão "Excluir Selecionados" desabilitado (sem permissão de exclusão)');
    }
    ...
    console.log('✅ CRUD de SGC_TipoFornecedor v3.1 inicializado com permissões:', {
