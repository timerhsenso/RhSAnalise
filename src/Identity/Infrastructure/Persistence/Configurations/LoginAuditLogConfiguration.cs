// RhSensoERP.Identity/Infrastructure/Persistence/LoginAuditLogConfiguration.cs
...
public class LoginAuditLogConfiguration : IEntityTypeConfiguration<LoginAuditLog>
{
    public void Configure(EntityTypeBuilder<LoginAuditLog> builder)
    {
        ...
        // Chave Primária
        builder.HasKey(x => x.Id).HasName("PK_SEG_LoginAuditLog");
        ...
        builder.Property(x => x.LoginAttemptAt).HasColumnType("datetime2(7)").HasDefaultValueSql("GETUTCDATE()").IsRequired();
        builder.Property(x => x.IsSuccess).IsRequired();
        builder.Property(x => x.FailureReason).HasMaxLength(100).IsRequired(false);
        ...
        builder.Property(x => x.DeviceType).HasMaxLength(50).IsRequired(false);
        builder.Property(x => x.Location).HasMaxLength(100).IsRequired(false);
        builder.Property(x => x.TwoFactorUsed).HasDefaultValue(false).IsRequired();
